...
FIXTURE = os.path.join(os.path.dirname(__file__), "..", "fixtures", "caisse_epargne.pdf")


class TestCaisseEpargneParser:
    def setup_method(self):
        self.parser = CaisseEpargneParser()

    def test_detection(self):
        with pdfplumber.open(FIXTURE) as pdf:
            text = pdf.pages[0].extract_text()
        assert CaisseEpargneParser.can_detect(text) >= 0.5

    def test_parse_transaction_count(self):
        with pdfplumber.open(FIXTURE) as pdf:
            result = self.parser.parse(pdf.pages)
        assert result.bank_slug == "caisse-epargne"
    ...
    def test_parse_debits(self):
        ...
        assert amounts[1] == pytest.approx(-93.47)
        assert amounts[2] == pytest.approx(-45.00)
    ...
    def test_parse_credits(self):
        ...
        assert amounts[0] == pytest.approx(2.34)
        assert amounts[1] == pytest.approx(1234.00)
