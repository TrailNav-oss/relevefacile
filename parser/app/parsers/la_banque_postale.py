...
class LaBanquePostaleParser(BankParser):
    """Parser for La Banque Postale bank statements."""
    ...
    detection_patterns = [
        ...
        r"BANQUE\s+POSTALE",
        r"www\.labanquepostale",
        r"compte\s+courant\s+postal",
    ]
    ...
    def _extract_header(self, text: str, account: AccountInfo) -> None:
        ...
        if not account.number:
            # La Banque Postale uses CCP-style account numbers
            m = re.search(r"(?:postal|CCP)\s*:\s*([\d\w\s]+)", text, re.IGNORECASE)
            if m:
